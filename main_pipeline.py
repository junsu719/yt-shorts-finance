...
from scripts.chart_gen import generate_finance_chart, chart_to_clip, CHART_TYPES, generate_market_chart
...
def run(topic: str, custom_photos: dict[int, Path] | None = None) -> str:
    ...
    # 1. Generate script
    log.info("[1/4] 生成腳本...")
    custom_path = Path("custom_narration.txt")
    narration_override = custom_path.read_text(encoding="utf-8") if custom_path.exists() else None
    ...
    for slot_num, slot_spec in enumerate(slot_plan):
        ...
        if slot_spec["type"] == "chart":
            ...
            if not chart_success:
                ...
                fetch_clip(q, clip_path, source="auto", seen_ids=seen_ids)
            ...
            try:
                fetch_clip(q, clip_path, source=source, seen_ids=seen_ids)
